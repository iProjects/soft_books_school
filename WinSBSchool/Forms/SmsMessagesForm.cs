...
{
    public partial class SmsMessagesForm : Form
    {
        Repository rep;
        ...
        public SmsMessagesForm(string user, string Conn)
        {
            InitializeComponent();
        ...
        private void btnSendSms_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                SendSmsForm arf = new SendSmsForm(_user, connection) { Owner = this };
                arf.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void btnDelete_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (dataGridViewSms.SelectedRows.Count != 0)
            {
                try
                {
                    ...
                    if (DialogResult.Yes == MessageBox.Show("Are you sure you want to delete SMS\n" + _sms.UserDataText.ToUpper().ToString().Trim().ToUpper(), "Confirm Delete",
    MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                    {
                        rep.DeleteSmsMessageStore(_sms);
                        RefreshGrid();
                    }
                }
                catch (Exception ex)
        ...
        private void chkReadSms_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (chkReadSms.Checked)
        ...
        private void btnProcess_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                if (dataGridViewSms.SelectedRows.Count != 0)
                {
                    ...
                    //extract phone number, transaction amount
                    ...
                    rep.UpdateSmsMessageStore(message);
                    ...
                    foreach (var sms in messagearray)
                    {
                        ...
                        _Output += Environment.NewLine + "sms: " + sms + Environment.NewLine;
                    }
