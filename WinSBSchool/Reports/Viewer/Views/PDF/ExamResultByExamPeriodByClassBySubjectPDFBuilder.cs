...
{
    public class ExamResultByExamPeriodByClassBySubjectPDFBuilder
    {
        string sFilePDF;
        ExamResultByExamPeriodByClassBySubjectViewModel examResultByExamPeriodByClassBySubjectViewModel;
        string Message;
        Document document;
        ...
        public ExamResultByExamPeriodByClassBySubjectPDFBuilder(ExamResultByExamPeriodByClassBySubjectViewModel _examResultByExamPeriodByClassBySubjectViewModel, string FileName)
        {
            examResultByExamPeriodByClassBySubjectViewModel = _examResultByExamPeriodByClassBySubjectViewModel;
            sFilePDF = FileName;
        }
        ...
        private void AddDocHeader()
        {
            ...
            try
            {
                ...
                Cell RCell = new Cell(new Phrase(examResultByExamPeriodByClassBySubjectViewModel.ReportName, new Font(Font.HELVETICA, 15, Font.BOLD, Color.BLUE)));
                ...
                Image img0 = pdfgen.DoGetImageFile(examResultByExamPeriodByClassBySubjectViewModel.SchoolLogo);
                ...
                Logcell.Add(new Phrase(examResultByExamPeriodByClassBySubjectViewModel.SchoolSlogan, new Font(Font.HELVETICA, 8, Font.ITALIC, Color.BLACK)));
                examResultByExamPeriodByClassTable.AddCell(Logcell);

                Cell reportdateCell = new Cell(new Phrase("Report Date:     " + examResultByExamPeriodByClassBySubjectViewModel.ReportDate.ToString("MMMM  dd, yyyy"), hFont));
        ...
        private void AddYearTerm()
        {
            ...
            Table yrTable = new Table(4);
            yrTable.WidthPercentage = 50;
            yrTable.Alignment = Table.ALIGN_LEFT;
            yrTable.Spacing = 1;
            yrTable.Padding = 1;

            Cell yearCell = new Cell(new Phrase("YEAR: " + examResultByExamPeriodByClassBySubjectViewModel.Year.ToString(), thFont));
            yearCell.Border = Cell.RECTANGLE;
            yearCell.HorizontalAlignment = Cell.ALIGN_CENTER;
            yrTable.AddCell(yearCell);

            Cell termCell = new Cell(new Phrase("TERM: " + examResultByExamPeriodByClassBySubjectViewModel.Term.ToString(), thFont));
            termCell.Border = Cell.RECTANGLE;
            termCell.HorizontalAlignment = Cell.ALIGN_CENTER;
            yrTable.AddCell(termCell);

            Cell examdateCell = new Cell(new Phrase("EXAM DATE: " + examResultByExamPeriodByClassBySubjectViewModel.ExamDate, thFont));
            examdateCell.Border = Cell.RECTANGLE;
            examdateCell.HorizontalAlignment = Cell.ALIGN_CENTER;
            yrTable.AddCell(examdateCell);

            Cell examtypeCell = new Cell(new Phrase("EXAM TYPE: " + examResultByExamPeriodByClassBySubjectViewModel.ExamTypeShortcode, thFont));
            examtypeCell.Border = Cell.RECTANGLE;
            examtypeCell.HorizontalAlignment = Cell.ALIGN_CENTER;
            yrTable.AddCell(examtypeCell);
        ...
        { 
            ...
            foreach (var item in examResultByExamPeriodByClassBySubjectViewModel.ExamResults)
            {
                AddExamPeriodTableDetails(item);
            }

        }
        private void AddExamPeriodTableDetails(ExamResultsByClassBySubjectDTO ercdto)
        {
            document.Add(new Phrase("\n"));
