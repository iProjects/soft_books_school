...
{
    public class ExamResultsByClassPDFBuilder
    {
        string sFilePDF;
        ...
        public ExamResultsByClassPDFBuilder(ExamResultsByClassViewModel _examResultsByClassViewModel, string FileName, string Conn)
        {
            try
            {
                ...
                if (_examResultsByClassViewModel == null)
                    throw new ArgumentNullException("ExamResultsByClassViewModel is null");
                _ViewModel = _examResultsByClassViewModel;
        ...
        private void AddDocBody()
        {
            try
            {
                if (_ViewModel._Subjects.Count() > 0)
                {
                    int Cols = 3 + _ViewModel._Subjects.Count() + 3;
        ...
        private void AddTableHeaders(Table resultsTable)
        {
            ...
            if (_ViewModel._Subjects.Count() > 0)
            {
                ...
                subjectCell.Colspan = _ViewModel._Subjects.Count();
            ...
            if (_ViewModel._Subjects.Count() > 0)
            {
                foreach (var r in _ViewModel._Subjects)
                {
        ...
        //table details 
        private void AddTableDetails(Table resultsTable, ExamResultsByClassDTO stRec)
        {
            try
            {
                ...
                if (_ViewModel._Subjects.Count() > 0)
                {
                    ...
                    {
                        var rec = stRec.StudentExamResults.Where(s => s.SubjectCode == sub).FirstOrDefault();

                        if (rec != null)
                        {
                            List<MarksDTO> mrks = rec.Marks.ToList();
                            double mark = mrks.Sum(i => i.Mark) ?? 0; 
                ...
                Cell meanMarkCell = new Cell(new Phrase(stRec.MeanMarks.ToString("F"), tdFont));
                meanMarkCell.Border = Cell.RECTANGLE;
                meanMarkCell.HorizontalAlignment = Cell.ALIGN_CENTER;
                resultsTable.AddCell(meanMarkCell);
        ...
        //table totals
        private void AddTableTotals(Table resultsTable)
        {

            try
            {
                Cell adminoCell = new Cell(new Phrase("", tdFont));
                adminoCell.Border = Cell.RECTANGLE;
                adminoCell.HorizontalAlignment = Cell.ALIGN_CENTER;
                resultsTable.AddCell(adminoCell);
                ...
                Cell totalCell = new Cell(new Phrase("", tdFont));
                totalCell.Border = Cell.RECTANGLE;
                totalCell.HorizontalAlignment = Cell.ALIGN_CENTER;
                resultsTable.AddCell(totalCell);

                Cell meanMarkCell = new Cell(new Phrase("", tdFont));
                meanMarkCell.Border = Cell.RECTANGLE;
                meanMarkCell.HorizontalAlignment = Cell.ALIGN_CENTER;
                resultsTable.AddCell(meanMarkCell);

                Cell gradeCell = new Cell(new Phrase("", tdFont));
                gradeCell.Border = Cell.RECTANGLE;
                gradeCell.HorizontalAlignment = Cell.ALIGN_CENTER;
                resultsTable.AddCell(gradeCell);
            }
